...
/**
 * THIS IS A UNCHANGED COPY FROM https://github.com/prometheus/client_java/blob/master/simpleclient_dropwizard/src/main/java/io/prometheus/client/dropwizard/samplebuilder/CustomMappingSampleBuilder.java
 * <p>
 * See discussion in https://github.com/prometheus/client_java/issues/518
 * <p>
 * Custom {@link SampleBuilder} implementation to allow Dropwizard metrics to be translated to Prometheus metrics
 * including custom labels and names. Prometheus metric name and labels are extracted from the Dropwizard name based on
 * the provided list of {@link MapperConfig}s. The FIRST matching config will be used. If no config is matched, the
 * {@link DefaultSampleBuilder} is used.
 */
public class CustomMappingSampleBuilder implements SampleBuilder {
    ...
    public CustomMappingSampleBuilder(final List<MapperConfig> mapperConfigs) {
        ...
        compiledMapperConfigs = new ArrayList<CompiledMapperConfig>(mapperConfigs.size());
        for (MapperConfig config : mapperConfigs) {
            compiledMapperConfigs.add(new CompiledMapperConfig(config));
        }
    }
    ...
    public Collector.MetricFamilySamples.Sample createSample(final String dropwizardName, final String nameSuffix, final List<String> additionalLabelNames, final List<String> additionalLabelValues, final double value) {
        ...
        for (CompiledMapperConfig config : compiledMapperConfigs) {
    ...
    static class CompiledMapperConfig {
        ...
        CompiledMapperConfig(final MapperConfig mapperConfig) {
            ...
            pattern = new GraphiteNamePattern(mapperConfig.getMatch());
        }
    }
