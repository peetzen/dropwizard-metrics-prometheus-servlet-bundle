package de.peetzen.dropwizard.metrics.prometheus.repackaged;
...
/**
 * THIS IS A MODIFIED COPY FROM https://github.com/prometheus/client_java/blob/master/simpleclient_dropwizard/src/main/java/io/prometheus/client/dropwizard/samplebuilder/MapperConfig.java
 * <p>
 * See discussion in https://github.com/prometheus/client_java/issues/518
 * <p>
 * POJO containing info on how to map a graphite metric to a prometheus one. Example mapping in yaml format:
 * <p>
 * match: test.dispatcher.*.*.* name: dispatcher_events_total labels: action: ${1} outcome: ${2}_out processor: ${0}
 * status: ${1}_${2}
 * <p>
 * Dropwizard metrics that match the "match" pattern will be further processed to have a new name and new labels based
 * on this config.
 */
public class MapperConfig {
    ...
    /**
     * Regex used to match incoming metric name. Uses a simplified glob syntax where only '*' are allowed. E.g:
     * org.company.controller.*.status.* Will be used to match org.company.controller.controller1.status.200 and
     ...
     * metric name. E.g.: match: test.dispatcher.*.*.* name: dispatcher_events_total_${1}
     * <p>
     * A metric "test.dispatcher.old.test.yay" will be converted in a new metric with name
     * "dispatcher_events_total_test"
     */
    private String name;

    /**
     * Labels to be extracted from the metric name. They should contain placeholders to be replaced with actual values
     * from the incoming metric name. Placeholders are in the ${n} format where n is the zero based index of the group
     * to extract from the original metric name. E.g.: match: test.dispatcher.*.* name: dispatcher_events_total_${0}
     * labels: label1: ${1}_t
     * <p>
     * A metric "test.dispatcher.sp1.yay" will be converted in a new metric with name "dispatcher_events_total_sp1" with
     * label {label1: yay_t}
     * <p>
...
// Disabled for now to allow use of * as a wildcard in more cases
// See discussion in https://github.com/prometheus/client_java/issues/518
